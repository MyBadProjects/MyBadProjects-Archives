...
module.exports = {
    name: "ultratoiletdeepfry",
    description: ":b:oi :joy::ok_hand: :toilet:",
    type: "funny",
    async execute(msg) {
        ...
        try {
            ...
            for (let i = 0; i < 10; i++) { desaturate(ctx, -20, 0, 0, data.width, data.height); }
            for (let i = 0; i < 10; i++) { contrast(ctx, 0, 0, data.width, data.height); }
            invert(ctx, 1, 1, data.width, data.height);
